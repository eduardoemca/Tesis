...
$sql="SELECT c.fecha_registro,c.codigo_compra,c.codigo_orden_compra,
			p.identificacion,p.razon_social,
			dc.codigo_producto,dc.cantidad_comprada,dc.precio_compra,pr.nombre,i.iva FROM compra c INNER JOIN 
			detalle_compra dc ON dc.codigo_compra = c.codigo_compra INNER JOIN proveedor p ON 
			c.codigo_proveedor = p.identificacion INNER JOIN producto pr ON dc.codigo_producto = pr.codigo_producto INNER JOIN iva i  ON  i.id_iva = pr.porcentaje_iva WHERE  c.fecha_registro BETWEEN '$fecha_desde' AND '$fecha_hasta'";
