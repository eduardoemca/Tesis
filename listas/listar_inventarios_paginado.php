<?php
	...
	if($action == 'cargar')
	{
		...
		$aColumnas = array('id_inventario','descripcion');//Columnas de busqueda
		$sTabla = "inventario";
		$sDonde = "";
		...
		$sDonde.=" ORDER BY id_inventario";

		include '../paginacion/paginacion_inventarios.php'; //incluir el archivo de paginación
		...
		if ($numero_filas>0){
			...
			while($fila = mysqli_fetch_array($sql)){
				...
				<tr>
					...
					<?php 
					$idinventario = $fila['id_inventario'];
					echo "<td class='text-center'>".'<button type="button" class="btn btn-info btn-venta btn-agregar-pro" name="btn_agregar_pro" id="btn_agregar_pro_iden" onclick="verificarcodigo_inventario('."'$idinventario'".')"><i class="glyphicon glyphicon-ok"></i></button>'."</td>"
					?>
				</tr>
				<?php
				}
