<?php
	...
	if($action == 'cargar')
	{
		...
		{
			$sDonde.="WHERE estado = '".$estado."' ORDER BY identificacion ASC";
		}
		else
		{
			$sDonde = "WHERE (";
				...
				<table class="table table-hover">
					...
					while($fila = mysqli_fetch_array($sql)){
						...
						<tr>
							...
							<?php 
							$idproveedor = $fila['identificacion'];
							echo "<td class='text-center'>".'<button type="button" class="btn btn-info btn-venta btn-agregar-pro" name="btn_agregar_pro" id="btn_agregar_pro_iden" onclick="verificarcodigo_proveedor('."'$idproveedor'".')"><i class="glyphicon glyphicon-ok"></i></button>'."</td>";
							?>
						</tr>
						<?php
						}
